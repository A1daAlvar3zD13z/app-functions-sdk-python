...
from src.app_functions_sdk_py.bootstrap.metrics.deletable_metrics_registry import (
    DeletableMetricsRegistry,
)
from src.app_functions_sdk_py.bootstrap.metrics.gauge_float64 import GaugeFloat64
from src.app_functions_sdk_py.bootstrap.metrics.reporter import (
    SERVICE_NAME_TAG_KEY,
    MessageBusReporter,
    COUNTER_COUNT_NAME,
    GAUGE_VALUE_NAME,
    TIMER_COUNT_NAME,
    ...
    TIMER_STDDEV_NAME,
    TIMER_VARIANCE_NAME,
    HISTOGRAM_COUNT_NAME,
    HISTOGRAM_MIN_NAME,
    HISTOGRAM_MAX_NAME,
    HISTOGRAM_MEAN_NAME,
    HISTOGRAM_STDDEV_NAME,
    HISTOGRAM_VARIANCE_NAME,
    GAUGE_FLOAT64_VALUE_NAME,
)
...
from src.app_functions_sdk_py.contracts.common.constants import (
    DEFAULT_BASE_TOPIC,
    METRICS_PUBLISH_TOPIC,
)
from src.app_functions_sdk_py.contracts.common.utils import build_topic
from src.app_functions_sdk_py.contracts.dtos.metric import (
    MetricTag,
    MetricField,
    Metric,
)
...
class TestNewMessageBusReporter(unittest.TestCase):
    def test_new_message_bus_reporter(self):
        ...
        expected_base_topic = build_topic(
            DEFAULT_BASE_TOPIC, METRICS_PUBLISH_TOPIC, expected_service_name
        )

        expected_telemetry_config = TelemetryInfo(
            Interval="30s", Metrics={"MyMetric": True}, Tags=None
        )

        expected_single_tag = [
            MetricTag(name=SERVICE_NAME_TAG_KEY, value=expected_service_name)
        ]
        expected_multi_tags = expected_single_tag + [
            MetricTag(name="gateway", value="my-gateway")
        ]
        ...
        tests = [
            TestData(
                "Happy path no additional tags",
                expected_service_name,
                None,
                expected_single_tag,
            ),
            TestData(
                "Happy path with additional tags",
                expected_service_name,
                gateway_tag,
                expected_multi_tags,
            ),
    ...
    def test_message_bus_reporter_report(self, mock_messaging_client_from):
        ...
        tests = [
            ...
            TestData(
                "Happy path - GaugeFloat64",
                gauge_float64,
                expected_gauge_float64_metric,
                False,
            ),
            TestData("Happy path - Timer", timer, expected_timer_metric, False),
            TestData(
                "Happy path - Histogram", histogram, expected_histogram_metric, False
            ),
        ...
        for test in tests:
            with self.subTest(test.name):
                ...
                def publish_side_effect(*args):
                    ...
                    ), "metricArg should be of type MessageEnvelope"
                    actual = Metric.from_dict(message.payload)
                    assert (
                        actual.name == test.expected_metric.name
                    ), "Metric name mismatch"

                    actual.timestamp = test.expected_metric.timestamp

                    assert (
                        actual == test.expected_metric
                    ), "Expected metric doesn't match actual"

                    topic_arg = args[1]
                    assert topic_arg is not None, "Topic argument should not be None"
                    assert (
                        topic_arg == expected_topic
                    ), "Topic doesn't match expected value"
