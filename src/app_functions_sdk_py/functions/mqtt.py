...
class MQTTSender:
    ...
    def __init__(
        ...
        self.mqtt_size_metrics = meters.Histogram(
    ...
    def _initialize_mqtt_client(self, lc: Logger, sp: SecretProvider) -> EdgeX | None:
        ...
        with self._lock:
            if (
                ...
                and self.secrets_last_retrieved >= sp.secrets_last_updated()
            ):
                return None

            lc.info("Initializing MQTT Client")

            mqtt_client_factory = MQTTFactory(
                sp,
                lc,
    ...
    def _connect_to_broker(self, lc: Logger) -> EdgeX | None:
        ...
        with self._lock:
            ...
            try:
                ...
                lc.info(
                    f"Successfully send CONNECT packet to MQTT Broker"
    ...
    def pre_connect_to_broker(
        self,
        lc: Logger,
        sp: SecretProvider,
        pre_connect_retry_count: int,
        retry_interval: int,
    ):
        """
        ...
        if self._client is None:
            ...
            if init_err is not None:
                lc.error(
                    f"Failed to pre-connect to MQTT Broker: {init_err}. "
        ...
        if self._client.is_connected():
            lc.info(
                "Already connected to MQTT Broker, so skip the pre-connect attempt."
            )
            return
        ...
        for i in range(pre_connect_retry_count):
            ...
            try:
                ...
                lc.warn(
                    f"Failed to pre-connect to MQTT Broker on attempt {i + 1}: {e}."
                    f" trying again in {retry_interval} seconds."
                )
                time.sleep(retry_interval)

        lc.error(
            f"Failed to pre-connect to MQTT Broker after attempting "
    ...
    def mqtt_send(self, ctx: AppFunctionContext, data: Any) -> Tuple[bool, Any]:
        ...
        if (
            ...
            or self.secrets_last_retrieved < secret_provider.secrets_last_updated()
        ):
            error = self._initialize_mqtt_client(lc, secret_provider)
            if error is not None:
                lc.error(
                    f"failed to initialize MQTT client in pipeline '{ctx.pipeline_id()}': "
        ...
        register_metric(
            ctx,
            lambda: f"{MQTT_EXPORT_ERRORS_NAME}-{tag_value}",
            lambda: self.mqtt_error_metrics,
            tag,
        )

        register_metric(
            ctx,
            lambda: f"{MQTT_EXPORT_SIZE_NAME}-{tag_value}",
            lambda: self.mqtt_size_metrics,
            tag,
        )
        ...
        if not self._client.is_connected():
            ...
            if error is not None:
                ...
                lc.error(
                    f"failed to connect to Broker in pipeline '{ctx.pipeline_id()}': "
                    f"{error.debug_messages()}"
                )
        ...
        result = self._client.publish(
            publish_topic, export_data, self._mqtt_config.qos, self._mqtt_config.retain
        )
        ...
        lc.debug(
            f"Sent {len(export_data)} bytes of data to MQTT Broker in pipeline "
            f"'{ctx.pipeline_id()}' to topic '{publish_topic}'"
        )
        lc.trace(
            f"Data exported to MQTT Broker in pipeline '{ctx.pipeline_id()}': "
