...
from ..trigger.defaultservicebinding import (
    DefaultTriggerServiceBinding,
    DefaultTriggerMessageProcessor,
)
from ...bootstrap.container.metrics import (
    MetricsManagerInterfaceName,
    metrics_manager_from,
)
...
from ...bootstrap.container.clients import (
    EventClientName,
    CommandClientName,
    ReadingClientName,
    DeviceServiceClientName,
    event_client_from,
    reading_client_from,
    command_client_from,
    device_service_client_from,
    DeviceProfileClientName,
    DeviceClientName,
    device_profile_client_from,
    device_client_from,
)
...
from ...bootstrap.container.logging import (
...
from ...contracts.common.constants import (
    CONFIG_STEM_APP,
    CORE_DATA_SERVICE_KEY,
    FUNCTION_KEYS_REPLACEMENTS,
    CORE_COMMAND_SERVICE_KEY,
...
class Service(ApplicationService):
    ...
    def __init__(
        ...
        self.service_key = (
            self.flags.service_key()
            if self.flags.service_key() is not None
            else (service_key)
        )
        self.service_config = ConfigurationStruct()
        self.web_server = server.WebServer(
            self._logger, self.service_key, self.service_config
        )
        self.runtime = None
        self.deferred_funcs = []
    ...
    def add_custom_route(
        ...
        if use_auth:
            ...
            self.web_server.add_route(
                route, handler, authentication_hook, methods=methods
            )
            return
    ...
    def initialize(self):
        ...
        """
        self._dic = Container()
        self._dic.update({LoggingClientInterfaceName: lambda get: self._logger})
        ...
        secret_provider = new_secret_provider(
            self.service_config,
            self.ctx_done,
            startup_timer,
            self._dic,
        ...
        try:
            cp.process(
                self.service_key, CONFIG_STEM_APP, self.service_config, secret_provider
            )
        except Exception as e:  # pylint: disable=broad-except
            self._logger.error(f"failed to process configuration: {e}")
            raise e

        self.runtime = FunctionsPipelineRuntime(
            self.service_key, self.target_type, self._dic
        )
        ...
        if env_use_registry or (self.flags.registry() and not was_overridden):
            try:
                registry_client = register_with_registry(
                    self.ctx_done,
                    startup_timer,
                    self.service_config,
                    self._logger,
            ...
            def deferred():
                ...
                try:
                    ...
                    self._logger.error(
                        "Unable to Un-Register service from the Registry",
                        "error",
                        str(err),
                    )
    ...
    def get_application_setting(self, key: str) -> str:
        ...
        if self.service_config.ApplicationSettings is None:
            raise ValueError(
                f"{key} setting not found: ApplicationSettings section is missing"
            )
    ...
    def add_functions_pipeline_for_topics(
        self, pipeline_id: str, topics: List[str], *functions: AppFunction
    ):
        """
    ...
    def setup_trigger(self, trigger_info: TriggerInfo) -> Trigger | None:
        ...
        message_processor = DefaultTriggerMessageProcessor(
            service_binding, self.metrics_manager()
        )
        ...
        if trigger_info.Type.upper() == TRIGGER_TYPE_HTTP:
            return HttpTrigger(
                service_binding, message_processor, self.web_server.router
            )
        ...
        if len(trigger_info.Type) == 0:
            self._logger.error(
                "trigger type not found, missing common config? "
                "Use -cp or -cc flags for common config"
            )
        else:
            self._logger.error(
                f"invalid Trigger type of '{trigger_info.Type}' specified"
            )
    ...
    def register_custom_trigger_factory(
        ...
        if nu in [TRIGGER_TYPE_MESSAGEBUS, TRIGGER_TYPE_HTTP, TRIGGER_TYPE_MQTT]:
            raise ValueError(
                f"cannot register custom trigger for builtin type ({name})"
            )
    ...
    def publish(self, data: Any, content_type: str):
        ...
        self.publish_with_topic(
            self.service_config.Trigger.PublishTopic, data, content_type
        )
    ...
    def _exit(self, signum, frame):
        ...
        self._logger.info(
            f"Service {self.service_key} received signal {signum} and frame {frame}, "
            f"prepare to exit."
        )
    ...
    def _initialize_message_client(self, startup_timer: Timer):
        ...
        if message_bus_info.Disabled:
            self._logger.info(
                "MessageBus is disabled in configuration, skipping setup."
            )
            return

        if (
            len(message_bus_info.Host) == 0
            or message_bus_info.Port == 0
            or len(message_bus_info.Protocol) == 0
            or len(message_bus_info.Type) == 0
        ):
            raise ValueError(
                "MessageBus configuration is incomplete, missing common config? Use "
        ...
        while startup_timer.has_not_elapsed():
            ...
            try:
                ...
                self._dic.update({MessagingClientName: lambda get: messaging_client})
                break
    ...
    def _create_custom_trigger(
        self, factory: Callable[[TriggerConfig], Trigger]
    ) -> Trigger:
        service_binding = DefaultTriggerServiceBinding(self.runtime, self)
        message_processor = DefaultTriggerMessageProcessor(
            service_binding, self.metrics_manager()
    ...
    def _initialize_service_clients(self, startup_timer: Timer):
        ...
        for service_key, service_info in self.service_config.Clients.items():
            if not service_info.UseMessageBus:
                try:
                    service_base_url = self._get_client_url(
                        service_key, service_info.url(), startup_timer
                    )
                except EdgeX as err:
            ...
            if service_key == CORE_DATA_SERVICE_KEY:
                self._create_event_client(
                    service_base_url, jwt_secret_provider, enable_name_field_escape
                )
                self._create_reading_client(
                    service_base_url, jwt_secret_provider, enable_name_field_escape
                )
            ...
            if service_key == CORE_METADATA_SERVICE_KEY:
                self._create_device_service_client(
                    service_base_url, jwt_secret_provider, enable_name_field_escape
                )
                self._create_device_profile_client(
                    service_base_url, jwt_secret_provider, enable_name_field_escape
                )
                self._create_device_client(
                    service_base_url, jwt_secret_provider, enable_name_field_escape
                )
    ...
    def _initialize_service_metrics(self):
        ...
        try:
            ...
            raise errors.new_common_edgex(errors.ErrKind.CONTRACT_INVALID, "msg", err)
        ...
        if disable_report:
            lc.info(
                "0 specified for metrics reporting interval. Setting to a very large duration "
        ...
        reporter = MessageBusReporter(
            lc, base_topic, self.service_key, self._dic, telemetry_config
        )
        ...
        self._dic.update({MetricsManagerInterfaceName: lambda get: manager})

    def _get_client_url(
        self, service_key: str, default_url: str, startup_timer: Timer
    ) -> str:
        """
        ...
        while startup_timer.has_not_elapsed():
            try:
                ...
                self._logger.warn(
                    f"failed to get service endpoint from registry: {err}. "
                    f"retrying..."
                )
                startup_timer.sleep_for_interval()
    ...
    def _create_event_client(
        ...
        self._dic.update({EventClientName: lambda get: event_client})

    def _create_reading_client(
        self,
        base_url: str,
        auth_injector: AuthenticationInjector,
        enable_name_field_escape: bool,
        ...
        self._dic.update({ReadingClientName: lambda get: reading_client})

    def _create_command_client(
        self,
        base_url: str,
        auth_injector: AuthenticationInjector,
        enable_name_field_escape: bool,
        ...
        self._dic.update({CommandClientName: lambda get: command_client})

    def _create_device_service_client(
        self,
        base_url: str,
        auth_injector: AuthenticationInjector,
        enable_name_field_escape: bool,
        ...
        self._dic.update({DeviceServiceClientName: lambda get: ds_client})

    def _create_device_profile_client(
        self,
        base_url: str,
        auth_injector: AuthenticationInjector,
        enable_name_field_escape: bool,
        ...
        self._dic.update({DeviceProfileClientName: lambda get: dp_client})

    def _create_device_client(
        self,
        base_url: str,
        auth_injector: AuthenticationInjector,
        enable_name_field_escape: bool,
        ...
        self._dic.update({DeviceClientName: lambda get: device_client})

    def _find_matching_function(
        self, configurable: Any, function_name: str
        ...
        if function_value is None:
            ...
            for method_name, method in inspect.getmembers(
                ...
                # name
                if (
                    function_name.startswith(method_name)
                    and len(method_name) > longest_matched_name_length
                ):
    ...
    def load_configurable_function_pipelines(self) -> dict[str, FunctionPipeline]:
        ...
        match target_type:
            ...
            case _:
                raise errors.new_common_edgex(
                    errors.ErrKind.CONTRACT_INVALID,
                    f"pipeline TargetType of '{target_type}' is not " f"supported",
                )
        ...
        if (
            len(default_execution_order) == 0
            and len(pipeline_config.PerTopicPipelines) == 0
        ):
            raise errors.new_common_edgex(
                errors.ErrKind.CONTRACT_INVALID,
        ...
        if len(default_execution_order) > 0:
            self._logger.debug(
                f"Default Function Pipeline Execution Order: "
            ...
            for _, per_topic_pipeline in pipeline_config.PerTopicPipelines.items():
                ...
                function_names = list(
                    convert_dict_keys_to_snake_case(
                        {k: None for k in function_names}, FUNCTION_KEYS_REPLACEMENTS
                    ).keys()
                )
    ...
    def _load_configurable_pipeline_transforms(
        self,
        pipeline_id: str,
        ...
        for function_name in execution_order:
            ...
            try:
                function_value, function_type = self._find_matching_function(
                    configurable, function_name
                )
    ...
    def load_custom_config(self, custom_config: Any, section_name: str):
        ...
        try:
            self._config_processor.load_custom_config_section(
                custom_config, section_name
            )
    ...
    def listen_for_custom_config_changes(
        ...
        threading.Thread(
            target=self._config_processor.listen_for_custom_config_changes,
            args=(config, section_name, changed_callback),
        ).start()
    ...
    def start_store_forward(self):
        ...
        self.runtime.start_store_and_forward(
            self.wait_group,
            self.ctx_done,
            self.store_forward_wait_group,
            self.store_forward_ctx_done,
            self.service_key,
        )
    ...
    def _initialize_store_client(self) -> Optional[errors.EdgeX]:
        ...
        if not self.service_config.Writable.StoreAndForward.Enabled:
            self._dic.update({StoreClientInterfaceName: lambda get: None})
            return None

        try:
            secrets = self.secret_provider().get_secrets(
                self.service_config.Database.Type
            )
        except errors.EdgeX as e:
        ...
        if err is not None:
            return errors.new_common_edgex(
                errors.kind(err),
                "initialize Database for Store and Forward failed",
                err,
            )

        self._dic.update({StoreClientInterfaceName: lambda get: store_client})
...
def _create_store_client(
    database: DatabaseInfo, _: Credentials, lc: Logger
) -> Optional[StoreClient]:
    """create store client"""
